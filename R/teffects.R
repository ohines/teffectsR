...
#'
#' \code{teffects} is used to fit treatment effect estimands of a binary exposure, on an outcome,
#' given a set of variables which adjust for confounding. Estimands included are: the Average Treatment Effect (\code{"ATE"}),
#' the Average Treatment Effect in the Treated (\code{"ATT"}), and the Average Treatment Effect in the Controls (\code{"ATC"}).
#' 
#' Several methods are implemented to adjust for confounding. Methods based on regression adjustment (\code{"\link[=fit_ra]{RA}","\link[=fit_aipw]{AIPW}"}) require 
#' that a glm outcome model is specified (See \code{\link[stats]{glm}}).
#' Methods base on the propensity score (\code{"\link[=fit_ipw]{IPW}","\link[=fit_aipw]{AIPW}","\link[=fit_psmatch]{PSMatch}"}) require that a binomial glm model is sepcified for the exposure (propensity score model).
#' When models are not required, dummy models must speficied, to parse the appropriate exposure/ outcome variable.
#' E.g. when using the method \code{"RA"}, then the exposure model might be \code{A~1}, where \code{A} is the binary exposure variable of interest.
...
#' that class) where the left hand side of the formula contains the outcome variable of interest. The right hand side should
#' not contain the exposure.
#' @param outcome.family family function for the outcome model, can be can be a character string naming a family function,
#' a family function or the result of a call to a family function. (See \link[stats]{family} for details of family functions.)
#' For methods \code{"IPW"} and \code{"PSMatching"} no outcome model is required.
#' @param treatment.effect The treatment effect estimand. Can be either \code{"ATE","ATT","ATC"} or \code{"All"} to fit all three.
...
#' @param method The mediation fitting method to be used. Can be either \code{"AIPW","IPW","RA"} or \code{"PSMatch"}
#' @param ... arguments to be passed to the matching function when using the \code{"PSMatch"} method. See \link[Matching]{Match} for details.
#'
#' @return An object of class \code{teffect} with effect estimates,
#'  estimated standard errors, Wald based test statistics and estimated potnetial outcome means.
#' @examples
...
teffect <- function(exposure.formula,outcome.formula,
  ...
  m <- match(c("exposure.formula","outcome.formula","data","weights"),names(mf), 0L)
  ...
  weights <- as.vector(model.weights(xf))
  xt <- attr(xf, "terms") # allow model.frame to have updated it
  Zx <-  model.matrix(xt, xf) 
  ...
  if (method %in% c("RA","AIPW") ){
    ...
    if(is.character(outcome.family))
      outcome.family <- get(outcome.family, mode = "function", envir = parent.frame())
  ...
  ## exposure family
  if (method=="AIPW"){
    a <- fit_aipw(X,Y,Zx,Zy,Ofam,treatment.effect = treatment.effect, weights=weights)
  }else if (method == "IPW"){
    if(treatment.effect != "ATE"){
      warning("Method: 'IPW' only compatible with ATE. Estimating ATE. ")
    }
    a <- fit_ipw(X,Y,Zx,weights=weights)
  }else if (method == "RA"){
    a <- fit_ra(X,Y,Zy,Ofam,treatment.effect = treatment.effect,weights=weights)
  } else if (method == "PSMatch"){
    a <- fit_psmatch(X,Y,Zx,
