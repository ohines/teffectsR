...
#' to \code{'PSMatch'}.
#' Propensity Score Matching fits a (binomial \link[stats]{glm}) model for the
#' exposure (propensity score model).
#' Observations from each treatment group are matched based on predictions from
#' this model.
...
fit_psmatch <- function(X, Y, Zx, treatment.effect = "ATE", weights = rep(1, N), ...) {
  ...
  run <- lapply(P.wts, function(TE) {
    ...
    if (Mod_match$ndrops.matches > 0) {
      warning(gettextf(
        "Observations dropped while estimating '%s' due to caliper = '%i'", TE,
